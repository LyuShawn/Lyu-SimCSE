...
def get_page_info(page_id_list):
    ...
    params = {
            ...
            "prop": "info|categories",  # 同时获取基础信息和分类数据
...
while True:
    page_list = MySQL.batch_get_sent_page_in_page_id(offset=offset, limit=limit)
    if not page_list:
        print('done')
        break
    offset += limit
    ...
    # 50个一组
    for i in tqdm(range(0, len(non_exist_keys), 50),desc='batch search'):
        keys = non_exist_keys[i:i+50]
        try:
